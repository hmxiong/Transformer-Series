"""This function is used for training and testing"""
import math
import os
import sys
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    metric_logger.add_meter('class_error', tools.SmoothedValue(window_size=1, fmt='{value:.2f}'))
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        loss_dict_reduced = tools.reduce_dict(loss_dict)
        ...
        losses.backward()
        if max_norm > 0:
            torch.nn.tools.clip_grad_norm_(model.parameters(), max_norm)
        optimizer.step()
...
# @torch.no_grad()
# def evaluate_dab(model, criterion, postprocessors, data_loader, base_ds, device, output_dir, wo_class_error=False, args=None, logger=None):
#     try:
#         need_tgt_for_training = args.use_dn
#     except:
#         need_tgt_for_training = False
...
#         savepath = osp.join(args.output_dir, 'results-{}.pkl'.format(tools.get_rank()))
